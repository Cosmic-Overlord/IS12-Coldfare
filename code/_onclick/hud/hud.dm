...
/datum/hud
	...
	var/obj/screen/atk_intent
	var/obj/screen/offense_intent
	var/obj/screen/defense_intent
	var/obj/screen/quick_intent
	var/obj/screen/aim_intent

	var/list/adding //Stuff that won't be made hidden.
	var/list/other //For stuff that can be made hidden.
		...
		for(var/gear_slot in H.species.hud.gear)
			...
			if(inventory_shown && hud_shown)
				switch(hud_data["slot"])
					...
					if(slot_tie)
						if(H.tie_slot) H.tie_slot.screen_loc = hud_data["loc"]
			else
				switch(hud_data["slot"])
					if(slot_shoes)
					...
					if(slot_tie)
						if(H.tie_slot) H.tie_slot.screen_loc = null
...
//I changed the DMF because this is bad and I hate it - Matt
