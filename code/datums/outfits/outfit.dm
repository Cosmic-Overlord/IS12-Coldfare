...
/decl/hierarchy/outfit/proc/equip_base(mob/living/carbon/human/H, var/equip_adjustments)
	...
	if(chest_holster)
		H.equip_to_slot_or_del(new chest_holster(H), slot_chest)
	if(belt)
	...
	if(neck)
		H.equip_to_slot_or_del(new neck(H), slot_tie)
	if(l_hand)
	...
	if((flags & OUTFIT_HAS_BACKPACK))
		...
		if(backpack && !(flags & OUTFIT_NO_BACKPACK))
			if(back)
	...
	if(H.species && !(OUTFIT_ADJUSTMENT_SKIP_SURVIVAL_GEAR & equip_adjustments) && !(flags & OUTFIT_NO_SURVIVAL_GEAR))
