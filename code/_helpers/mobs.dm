...
/proc/do_after(mob/user, delay, atom/target = null, needhand = 1, progress = 1, var/incapacitation_flags = INCAPACITATION_STUNNED|INCAPACITATION_RESTRAINED|INCAPACITATION_KNOCKOUT, var/same_direction = 0, var/stay_still = TRUE)
	if(!user)
		return 0
	...
	while (world.time < endtime)
		...
		if(!user || user.incapacitated(incapacitation_flags) || (same_direction && user.dir != original_dir))
			. = 0
			break

		if(stay_still && user.loc != original_loc)
			. = 0
			break
