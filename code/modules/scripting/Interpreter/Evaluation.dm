...
/n_Interpreter
	proc
		Eval(node/expression/exp)
			...
			else if(istype(exp, /node/expression/opperator))
		...
		EvalOperator(node/expression/opperator/exp)
			if(istype(exp, /node/expression/opperator/binary))
				var/node/expression/opperator/binary/bin=exp
				switch(bin.type)
					if(/node/expression/opperator/binary/Equal)
						return Equal(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/NotEqual)
						return NotEqual(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/Greater)
						return Greater(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/Less)
						return Less(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/GreaterOrEqual)
						return GreaterOrEqual(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/LessOrEqual)
						return LessOrEqual(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/LogicalAnd)
						return LogicalAnd(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/LogicalOr)
						return LogicalOr(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/LogicalXor)
						return LogicalXor(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/BitwiseAnd)
						return BitwiseAnd(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/BitwiseOr)
						return BitwiseOr(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/BitwiseXor)
						return BitwiseXor(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/Add)
						return Add(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/Subtract)
						return Subtract(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/Multiply)
						return Multiply(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/Divide)
						return Divide(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/Power)
						return Power(Eval(bin.exp), Eval(bin.exp2))
					if(/node/expression/opperator/binary/Modulo)
						return Modulo(Eval(bin.exp), Eval(bin.exp2))
					else
						RaiseError(new/runtimeError/UnknownInstruction())
			else
				switch(exp.type)
					if(/node/expression/opperator/unary/Minus)
						return Minus(Eval(exp.exp))
					if(/node/expression/opperator/unary/LogicalNot)
						return LogicalNot(Eval(exp.exp))
					if(/node/expression/opperator/unary/BitwiseNot)
						return BitwiseNot(Eval(exp.exp))
					if(/node/expression/opperator/unary/group)
						return Eval(exp.exp)
					else
						RaiseError(new/runtimeError/UnknownInstruction())
