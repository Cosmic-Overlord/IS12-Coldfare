...
*/
		Precedence(node/expression/opperator/top, node/expression/opperator/input)
			if(istype(top))
				top=top.precedence
			if(istype(input))
...
/*
	...
	- <GetUnaryOperator()>
*/
		GetOperator(O, type=/node/expression/opperator, L[])
...
*/
		GetBinaryOperator(O)
			return GetOperator(O, /node/expression/opperator/binary, options.binary_operators)
...
*/
		GetUnaryOperator(O)
			return GetOperator(O, /node/expression/opperator/unary,  options.unary_operators)
...
*/
		Reduce(stack/opr, stack/val)
			var/node/expression/opperator/O=opr.Pop()
			if(!O) return
			if(!istype(O))
			...
			if(istype(O, /node/expression/opperator/binary))
				var/node/expression/opperator/binary/B=O
				B.exp2=val.Pop()
				B.exp =val.Pop()
				val.Push(B)
			else
		...
		ParseExpression(list/end=list(/token/end), list/ErrChars=list("{", "}"))
			...
			while (TRUE)
				...
				if(istype(curToken, /token/symbol) && curToken.value=="(")			//Parse parentheses expression
					...
					var/node/expression/opperator/curOperator											//Figure out whether it is unary or binary and get a new instance.
...
*/
		ParseParenExpression()
			if(!CheckToken("(", /token/symbol))
				return
			return new/node/expression/opperator/unary/group(ParseExpression(list(")")))
