...
/mob/living/carbon/human/verb/quick_equip()
	...
	if(ishuman(src))
		...
		if(!I)
			...
			if(back_item)
				if(unEquip(back_item))
					put_in_active_hand(back_item)
			else
				to_chat(H, "<span class='notice'>You are not holding anything to equip.</span>")
			return
		if(I.time_to_equip)
			if(!do_after(src, I.time_to_equip, stay_still = FALSE))
				to_chat(src, "You stop putting on \the [I].")
				return 0
...
/mob/living/carbon/human/equip_to_slot(obj/item/W as obj, slot, redraw_mob = 1)
	...
	switch(slot)
		...
		if(slot_chest)
			src.chest_holster = W
			W.equipped(src, slot)
			update_inv_chest_holster(redraw_mob)
			if(W.equipsound)//So like a sword being sheathed.
				playsound(src, W.equipsound, 50, 1)
		if(slot_belt)
			src.belt = W
